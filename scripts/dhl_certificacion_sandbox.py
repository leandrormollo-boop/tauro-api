#!/usr/bin/env python3
"""Genera el paquete que DHL Sistemas pide para certificar la integración.

La ejecución está deliberadamente limitada al endpoint sandbox. Usa el mismo
``DHLClient.create_shipment`` que el portal y exporta, sin credenciales:

* el JSON exacto enviado a ``POST /shipments``;
* la respuesta técnica sin los documentos base64;
* la guía PDF;
* la factura comercial PDF;
* tracking, Message-Reference y hashes de los documentos.

Ejemplo:

    DHL_ENVIRONMENT=sandbox ... python scripts/dhl_certificacion_sandbox.py \
        --input var/dhl_certificacion/envio.json

Los puntos suspensivos representan variables cargadas de forma segura en el
entorno. Nunca pases usuario o contraseña como argumentos de línea de comando.
"""
...
def _json_sin_base64(data: object) -> object:
    ...
    for documento in salida.get("documents") or []:
        ...
        documento["content"] = {
            ...
            "sha256_base64": hashlib.sha256(contenido.encode("ascii", "ignore")).hexdigest(),
        }
    return salida
...
def generar_paquete(datos: dict, carpeta_base: Path) -> Path:
    ...
    _json_privado(destino / "01_codigo_enviado.json", codigo)
    _json_privado(
    ...
    if not guia or not bytes(guia).startswith(b"%PDF"):
        raise RuntimeError(f"DHL emitió tracking pero no devolvió la guía PDF. Revisar {destino}.")
    if not factura or not bytes(factura).startswith(b"%PDF"):
        raise RuntimeError(
            f"DHL emitió tracking pero no devolvió la factura comercial PDF. Revisar {destino}."
        )
    ...
    _json_privado(destino / "05_resumen.json", resumen)
...
def main() -> int:
    ...
    parser.add_argument("--input", required=True, type=Path,
                        help="JSON con shipper, recipient, bultos y tax_paga")
    parser.add_argument("--output-dir", type=Path,
                        default=ROOT / "var" / "dhl_certificacion")
