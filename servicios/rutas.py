...
def _row_a_ruta(r: dict) -> Ruta:
    # Los países SIEMPRE salen normalizados a ISO-2 ("ESTADOS UNIDOS" → "US").
    # La migración desde Sheets puede escribir nombres completos en la tabla;
    # normalizar acá blinda al cotizador, al wizard y a FedEx de datos sucios.
    return Ruta(
        ruta_id=str(r["ruta_id"]).strip(),
        origen_pais=pais_a_iso2(str(r["origen_pais"])),
        origen_ciudad=str(r["origen_ciudad"]).strip(),
        origen_zip=str(r["origen_zip"]).strip(),
        destino_pais=pais_a_iso2(str(r["destino_pais"])),
...
def find_ruta_por_paises(origen_pais: str, destino_pais: str) -> Optional[Ruta]:
    """
    Match en Python (son ~4-10 rutas) normalizando AMBOS lados a ISO-2:
    la tabla puede tener "ESTADOS UNIDOS" (migración Sheets) y el caller "US"
    — comparando en SQL crudo esos nunca matchearían.
    """
    o = pais_a_iso2(origen_pais)
    d = pais_a_iso2(destino_pais)
    if not o or not d:
        return None
    for r in get_rutas_activas():
        if r.origen_pais == o and r.destino_pais == d:
            return r
    return None
